	...
	async function loadRequests() {
		...
		try {
			...
			if (res.success && res.data) {
				requests = res.data.filter((r: PengajuanMataKuliah) => r.status === 'pending');
			}
		} catch (err: any) {
...
<Card style="padding: 24px; margin-top: 24px;">
	...
	{:else if requests.length === 0}
		<div class="state-container">Tidak ada pengajuan mata kuliah yang menunggu persetujuan.</div>
	{:else}
